function obj1_chemical_ground_truth_statistics_lmm(inputXlsx, sheetName)
% obj1_chemical_ground_truth_statistics_lmm
%
% Statistical analysis of chemical ground-truth responses for the grape dataset.
%
% This workflow:
%   1. reads the master chemistry table from Excel,
%   2. resolves and harmonises the experimental design factors,
...
%   5. exports manuscript-oriented tables, publication figures, and a run log.
%
% Statistical design:
%   - Global model: y ~ Date * Irrigation + (1|Block)
%   - Irrigated subset: y ~ Date * Management + (1|Block)
%   - Rainfed subset:  y ~ Date * Management + (1|Block)
%   - Fallback additive model when interaction terms are not estimable.
%
% Inputs:
%   inputXlsx  - path to the master Excel file
%   sheetName  - worksheet name (optional). If omitted or empty, the first
%                sheet is used.
%
% Output:
%   A time-stamped results folder named:
%       Objetivo_1/RESULTS_Obj1_YYYYMMDD_HHMMSS/
%   containing:
%       - O1_log.txt
%       - O1_Stats_Summary.xlsx
%       - O1_Table1_Manuscript.xlsx
%       - manuscript-oriented figures (.png and .fig)
%
% Notes:
%   - One row of the input table is expected per experimental unit.
%   - The script resolves endpoint and factor columns from candidate names.
